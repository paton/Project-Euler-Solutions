...
var createMatrixFromNumbers = function(numbers, x, y) {
  ...
  for (var i = 1; i <= x; i++) {
    matrix.push([]);
    for (var j = 1; j <= y; j++) {
      matrix[i-1][j-1] = numbers[(i - 1) * x + j - 1];
    }
  }
...
var findGreatestAdjacentProduct = function (matrix) {
  ...
  // Find greatest left-slanted diagnal
  for (i = 0; i < maxX - 3; i++) {
    for (j = 0; j < maxY - 3; j++) {
  ...
  // Find greatest right-slanted diagnal
  for (i = 3; i < maxX; i++) {
    for (j = 0; j < maxY - 3; j++) {
      product = matrix[i][j] * matrix[i - 1][j + 1] * matrix[i - 2][j + 2] * matrix[i - 3][j + 3];
      if (product > greatestProduct) greatestProduct = product;
...
console.log(findGreatestAdjacentProduct(createMatrixFromNumbers(numbers, 20, 20)));
